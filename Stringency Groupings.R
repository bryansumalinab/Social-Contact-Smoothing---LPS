...
data <- readRDS("data_stringency.RDS")
...
# Difference: non-intercept (high, moderate) - intercept (low)
diff_mat2 <- lograte_samples[idx_grp, , drop = FALSE] - 
  lograte_samples[idx_int, , drop = FALSE]

# Difference: high - moderate
diff_HM <- lograte_samples[idx_grp, , drop = FALSE][(1:block_size),] - 
  lograte_samples[idx_grp, , drop = FALSE][- (1:block_size),] 

# Combined differences
diff_mat3 <- rbind(diff_HM, diff_mat2)

# Mean difference, 2.5% and 97.5% quantiles
diff_all_summary <- t(apply(diff_mat3, 1, function(x) {
...
# Note: first block_size rows corresponds to high - moderate;
# Here, the rows group '(Intercept)' in the data rate_long 
diff_all <- cbind(rate_long, diff_all_summary)
...
diff_all$group <- factor(diff_all$group,
                         levels = c("groupmoderate",
                                    "grouphigh",
                                    "(Intercept)"))
group_labels_diff <- c(
  groupmoderate = "bold(Diff[ML])",
  grouphigh     = "bold(Diff[HL])",
  `(Intercept)`   = "bold(Diff[HM])"
)
...
plot_meandiff <- ggplot(diff_all, aes(age_part, age_cont, z = mean)) +
  ...
  scale_fill_viridis_d(option = "D", name = "Mean difference") +
  facet_wrap(~ group, labeller = as_labeller(group_labels_diff, label_parsed)) +
  labs(x = "Participant's age", y = "Contact's age") + gg_base + 
  theme(
...
plot_significance <- ggplot(diff_all, aes(age_part, age_cont, fill = sig_dir)) +
  ...
  scale_fill_manual(
    name   = "Significance of the difference\nat 5% level",
    values = c(
      "No"             = "grey80",
      "Yes (positive)" = "#4472c4",
      "Yes (negative)"   = "tomato"
...
plot_meandiff # Differences in contact rates
